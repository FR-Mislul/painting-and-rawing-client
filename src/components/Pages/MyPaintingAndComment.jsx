    ...
    return (
        <div className="w-11/12 mx-auto">
            ...
            <div className="flex items-start overflow-x-auto overflow-y-hidden sm:justify-start flex-nowrap text-gray-100">
                ...
                className={`flex items-center flex-shrink-0 px-5 py-3 space-x-2 ${location.pathname === '/myPaintingAndComment' ? 'border border-b-0 border-gray-300' : 'border-b border-gray-300'} rounded-t-lg ${theme === "dark" ? "text-gray-200" : "text-gray-600"}`}>
                    <span>My painting</span>
                </Link>
                <Link to={`comment`}
                // onClick={()=> setActiveTeb(false)}
                 className={`flex items-center flex-shrink-0 px-5 py-3 space-x-2 ${location.pathname.includes('comment') ? 'border border-b-0' : 'border-b'} rounded-t-lg ${theme === "dark" ? "text-gray-200" : "text-gray-600"}`}>
